            ...
            <div class="card">
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                  ...
                  <!-- <tr v-for="(khenthuong, index) in khenthuongs" :key="index"> -->
                  <tr v-for="khenthuong in khenthuongs" :key="khenthuong.id">
                    <td>{{khenthuong.id}}</td>
                    <!-- <td>{{index+1}}</td> -->
                    <td>{{khenthuong.tenkhenthuong}}</td>
                    <td>{{khenthuong.ghichu}}</td>
                    <td>{{khenthuong.created_at | showDate}}</td>
                    <td>
                      <a href="#" @click="editModal(khenthuong)">
                        <i class="fa fa-edit green"></i>
                      </a>
                      |
                      <a href="#" @click="deletekhenthuong(khenthuong.id)">
        ...
        <div class="modal fade" id="addNewModal" tabindex="-1" role="dialog" aria-labelledby="addNewModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
              ...
              <!-- Form -->
              <form @submit.prevent="editmode ? updatekhenthuong() : createkhenthuong()">
              <div class="modal-body">
                <div class="form-group">
                  <input v-model="form.tenkhenthuong" type="text" name="tenkhenthuong"
                    placeholder="Tên khen thưởng"
                    class="form-control" :class="{ 'is-invalid': form.errors.has('tenkhenthuong') }">
...
<script>
    export default {
        ...
        methods: {
          editModal(khenthuong){
            this.editmode = true;
            this.form.reset();
            $('#addNewModal').modal('show');
            this.form.fill(khenthuong);
          },
          newModal(){
            this.editmode = false;
            this.form.reset();
          ...
          updatekhenthuong(){
            this.$Progress.start();
            this.form.put('api/khenthuong/' + this.form.id)
            .then(() => {
          ...
          deletekhenthuong(id){
            swal({
              ...
              if (result.value) {
                this.form.delete('api/khenthuong/' + id)
                .then(() => {
          ...
          loadkhenthuongs(){
            this.$Progress.start();
            axios.get("api/khenthuong")
            .then(({ data }) => {
                (this.khenthuongs = data.data)
                this.$Progress.finish();
            })
            .catch((error) => {
              toast({
